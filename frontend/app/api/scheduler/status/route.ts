...
export async function GET() {
  try {
    const data = await backendFetch<Record<string, unknown>>("/api/v1/scheduler/status");
    return NextResponse.json(envelope(data, false));
  } catch (err) {
    const message = err instanceof BackendError ? err.message : "Scheduler status unavailable";
