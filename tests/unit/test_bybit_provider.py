"""Bybit public OHLCV provider with mocked exchange (no network)."""
...
async def test_fetch_ohlcv_maps_to_candles():
    ...
    provider = BybitProvider(exchange=exchange)
...
async def test_fetch_ohlcv_rejects_unsupported_symbol():
    ...
    with pytest.raises(ValueError, match="Unsupported symbol"):
        await provider.fetch_ohlcv("DOGE/USDT", "1m")
    await provider.close()
...
async def test_fetch_symbols_filters_supported():
    ...
    assert any(s.symbol == "BTC/USDT" for s in symbols)
    await provider.close()
