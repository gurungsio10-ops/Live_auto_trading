...
```bash
chmod +x scripts/codespaces_up.sh
...
# PGPASSWORD=atlas psql -h 127.0.0.1 -U atlas -d postgres -c 'CREATE DATABASE atlas'
alembic upgrade head

uvicorn app.main:app --host 0.0.0.0 --port 8000 --reload
# separate terminal:
cp frontend/.env.example frontend/.env.local   # ensure ATLAS_* match backend
