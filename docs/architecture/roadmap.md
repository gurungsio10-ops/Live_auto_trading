...
| 8 | Live Market Data (WebSocket) | **done** (Spot Testnet WS + RealWebSocketTransport) | Phase 7 |
...
## Truthful status corrections

Statuses track the actual implementation (see also
`docs/audit/phase16_repository_audit.md` for the Phase 16 paper baseline):

- **Phase 8 → done (Sprint 1):** `BinanceSpotTestnetWebSocket` + `RealWebSocketTransport`
  stream Spot Testnet kline/trade with reconnect, stale watchdog, dedup, and REST
  gap recovery. Public/production Binance live money feeds remain out of scope.
- **Phase 10 → partial:** readiness probes exist under `/api/v1/testnet/diagnostics`;
  some legacy readiness paths remain flag aggregators.
- **Phase 13 → partial:** `app/news/sentiment.py` is implemented and tested but is
  **not wired** into `RiskEngine`.
- **Phase 13.5 → done (Sprint 1):** factory selects paper/testnet; live raises
  `LiveTradingDisabledError`. Spot Testnet broker, runtime cycle, reconciler,
  dashboard APIs, and hydrate-on-start are wired. Live money trading stays disabled.

## Phase 16 / Sprint 1 acceptance — current state

Met: backend installs; `pytest` green; migrations apply; paper cycle + Spot Testnet
pipeline (sample + REST cycle + WS runtime); kill switch; live execution raises
`LiveTradingDisabledError`; CI coverage includes testnet/WS modules; frontend
lint/typecheck/build; docs under `docs/architecture/` and `docs/operations/`.

Still open (not live-ready): live order adapter, multi-day soak proof with real
testnet keys in CI, full journal-session wiring on every dashboard tick. Live
readiness checklist remains **unchecked**.
